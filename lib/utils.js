...
export const useTimer = (dateString) => {
  ...
  const launchDateUTC = Date.UTC(launchDate.getUTCFullYear(), launchDate.getUTCMonth(), launchDate.getUTCDate(), 0, 0, 0);

  useEffect(() => {
    function updateTimer() {
      const nowDate = new Date();
      var duration = launchDateUTC- Date.UTC(nowDate.getUTCFullYear(), nowDate.getUTCMonth(), nowDate.getUTCDate(), nowDate.getUTCHours(), nowDate.getUTCMinutes(), nowDate.getUTCSeconds());
